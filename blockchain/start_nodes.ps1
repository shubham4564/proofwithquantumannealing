...
Get-Process -Name "python" -ErrorAction SilentlyContinue | Where-Object {$_.CommandLine -like "*run_node.py*"} | Stop-Process -Force
...
function Start-Node {
    ...
    if ($nodeNum -eq 1) {
        ...
        if (Test-Path "keys\staker_private_key.pem") {
            ...
            Write-Host "⚠️  Warning: Key file $keyFile not found, using genesis key as fallback" -ForegroundColor Yellow
            $keyFile = "keys\genesis_private_key.pem"
        }
    }
...
if ($activeNodes -eq $NumNodes) {
    ...
    Write-Host "   ⚠️  Some nodes failed to start. Check logs for details." -ForegroundColor Yellow
} else {
